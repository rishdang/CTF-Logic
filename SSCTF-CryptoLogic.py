...
def encrypt1(k, p):
    ...
    for i in t2:
        out += encode(i)
    return out
#input char set and start logic
...
def decrypt(flagcrypt, flag):
    ...
    for d in range(27):
        for i in charset:
            xx = encrypt1(flagcrypt, f + i)
            if xx[:len(f+i)*8] == flag[:len(f+i)*8]:
                f=f+i
    print f
               
 
flagcrypt1 = chr(k0[0]) + chr(k1[0])+ chr(k2[0])+ chr(k3[0])+ chr(k4[0])+ chr(k5[0])+ chr(k6[0])+ chr(k7[0])
for i0 in k0:
    for i1 in k1:
        for i2 in k2:
            for i3 in k3:
